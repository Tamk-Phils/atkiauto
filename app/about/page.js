...
const AboutPage = () => {
  ...
  return (
    <div style={{ paddingBottom: '0rem', background: '#fff', color: '#0a0a0b', minHeight: '100vh' }}>
            ...
            {team.map((member, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}
                ...
                <div style={{ height: 240, background: '#f1f5f9', overflow: 'hidden' }}>
                  {/* member images are missing in typical new project, but we'll use placeholders or just handle missing src */}
                  <div style={{ width: '100%', height: '100%', background: '#cbd5e1', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontWeight: 800 }}>
